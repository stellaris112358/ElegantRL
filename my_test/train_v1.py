...
from singlepursuitEnv import PointChasingVecEnv
...
class RecordEpisodeStatisticsTorch(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # terminated: float tensor (0./1.), truncated: bool tensor
        terminated_bool = terminated.type(torch.bool)
        dones = torch.logical_or(terminated_bool, truncated)
        dones = dones.type(torch.int32)
        ...
        # Return (observations, rewards, dones, infos) to match main training loop expectation
...
if __name__ == "__main__":
    ...
    # env setup
    envs = PointChasingVecEnv(
        seed=args.seed,
        env_num=args.num_envs,
        sim_device=0 if device.type == "cuda" else -1
    )
